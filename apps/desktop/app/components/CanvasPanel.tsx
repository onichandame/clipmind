...
export function CanvasPanel({ outline }: CanvasPanelProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "prose prose-zinc prose-invert max-w-none focus:outline-none",
      },
    },
    onUpdate: ({ editor }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-zinc-950">
      {/* 顶部状态栏 */}
      <div className="h-14 border-b border-zinc-800/50 flex items-center justify-between px-6 bg-zinc-900/20 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium text-zinc-400">当前视图:</span>
          <div className="flex bg-zinc-800/50 p-1 rounded-lg border border-zinc-700/30">
        ...
        {activeMode === "outline" ? (
          (outline || outlineContent) ? (
            ...
            <div className="flex flex-col items-center justify-center h-full max-w-md mx-auto text-center px-8 -mt-20">
              ...
              <h3 className="text-xl font-semibold text-zinc-100 mb-2">等待灵感降临...</h3>
