...
app.post("/", async (c) => {
  ...
  dynamicSystemPrompt += `**【反问规范】**: 只要你的下一步是向用户提问且答案可以枚举（包括 yes/no、是否继续、是否确认这类确认型问题），都必须调用 \`ask_user_question\`，禁止把问题用纯文本抛出来等用户敲字。同一思考里若有多个独立分叉，把它们一起塞进 \`questions\` 数组里一次问完（最多 5 个），避免连续多轮反问。每个问题的 \`options\` 不要写"其他/自定义"——前端会自动追加一个开放式输入框选项。选项没有隐藏 id/value，只有 \`label\`；用户看到什么，下一轮你收到的答案就是什么，所以 \`label\` 必须是自然语言答案，禁止写 douyin/story_driven 这类机器枚举值。调用后必须立即停步，由用户点击提交触发下一轮。**唯一例外**：纯开放式问题（如"你想加什么字幕文案？"）无法预设选项时才用纯文本提问。\n`;
  ...
  const result = streamText({
    ...
    tools: {
      ...
      ask_user_question: tool({
        description:
          "**只要你的下一步是在向用户提问且答案可以被枚举出来（包括 yes/no 确认）**，就必须调用本工具，前端会渲染按钮卡片让用户一键回答，避免敲键盘。" +
          "**支持一次问多个问题**：把所有需要用户同时决策的分叉打包进 questions 数组（最多 5 个），避免连续多轮反问。" +
          "**重要**：每个问题的选项**末尾会被前端自动追加一个\"其他/自由输入\"项**，你**不需要也不能**在 options 里加这一项。" +
          "**选项字段只有 label，没有 value/id**：label 既是按钮文案，也是用户点击后提交给你的答案。必须写用户和你都能直接理解的自然语言，禁止写内部枚举值。" +
          "**典型场景**：缺大纲是先写还是直接生成方案 / 节奏紧凑还是舒缓 / 推进路线分叉 / 是否继续 / 是否确认 / 是/否。" +
          "**唯一禁用场景**：纯开放式问题（'你打算讲什么主题？'、'你想加什么字幕文案？'）——无法预设选项时才用纯文本提问。" +
        ...
        inputSchema: z.object({
          questions: z.array(z.object({
            ...
            options: z.array(z.object({
              label: z.string().min(1).describe('按钮主文案，同时也是点击后提交给模型的答案；必须是自然语言，禁止使用内部 id/枚举值'),
              description: z.string().optional().describe('按钮下的灰色小字补充说明，可省略'),
            }).strict()).min(2).max(6).describe('候选选项；不要包含"其他/自由输入"项，前端会自动注入'),
          }).strict()).min(1).max(5),
        }).strict(),
