...
app.post("/", async (c) => {
  ...
  dynamicSystemPrompt += `**【素材缺失保护规则】**: 当用户要求基于素材生成剪辑方案时，若 \`search_clips\` 或 \`search_assets\` 返回结果为空（无任何匹配素材），你必须立即停止方案生成流程，向用户说明缺少相关素材，并引导用户先上传或检索所需素材。严禁在没有检索到任何相关素材的情况下凭空生成剪辑方案。\n\n`;
